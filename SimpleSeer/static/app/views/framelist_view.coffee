  ...
  addObjs: (d)=>
    ...
    if @page == "tabImage"
      ...
      for o in d.models
        ...
        row = "<tr><td>"+dt.format("MM/DD/YYYY hh:mm a (UTC)")+"</td><td>"+(f.head_width_mm||_empty)+"</td><td>"+(f.shaft_width_mm||_empty)+"</td><td>"+(f.fillet_left_r||_empty)+"</td><td>"+(f.fillet_right_r||_empty)+"</td></tr>"
        row = $(row)
        resort = true; 
